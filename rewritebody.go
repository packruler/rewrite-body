...
func (bodyRewrite *rewriteBody) ServeHTTP(response http.ResponseWriter, req *http.Request) {
	...
	wrappedWriter := &httputil.HTTPWrapper{
		Request:        req,
	...
	if isSupported := wrappedWriter.SupportsProcessing(); !isSupported {
		return
	}

	log.Printf("Buffer bytes: %s", wrappedWriter.BodyBytes())

	bodyBytes, err := wrappedWriter.GetContent(math.MaxInt32)
	if err != nil {
	...
	// }
	wrappedWriter.SetContent(bodyBytes, wrappedWriter.GetContentEncoding())
}
